...
def get_upcoming_birthdays(users, days=7):
    ...
    for user in users:
        ...
        if birthday_this_year < today:
            # Якщо день народження вже минув, розглядаємо дату наступного року
            birthday_this_year = birthday_this_year.replace(year=today.year + 1)

        # Переносимо дату привітання на наступний робочий день, якщо потрібно
        birthday_this_year = adjust_for_weekend(birthday_this_year)

        # Перевіряємо, чи день народження впадає в період вперед на days днів
...
users = [
    ...
    {"name": "Sarah Lee", "birthday": "1957.4.22"},
